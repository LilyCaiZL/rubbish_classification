...
add_arg('dataset',          str,   'coco2014', "dataset can be coco2014, coco2017, and pascalvoc.")
...
def train(args,
    ...
    def save_model(postfix, feeded_var_names,target_vars,exe,main_prog):
        model_path = os.path.join(model_save_dir, postfix)
        if os.path.isdir(model_path):
            shutil.rmtree(model_path)
        print('save models to %s' % (model_path))
        fluid.io.save_inference_model(model_path,feeded_var_names,target_vars,exe, main_program=main_prog)
    ...
    def test(epoc_id, best_map):
        ...
        if test_map[0] > best_map:
            best_map = test_map[0]
            #modified
            save_model('best_model',image.name,nmsed_out,exe,test_prog)
    ...
    for epoc_id in range(epoc_num):
        ...
        if epoc_id % 10 == 0 or epoc_id == epoc_num - 1:
            ...
            #save_model(str(epoc_id), image.name, nmsed_out, exe, train_prog)
