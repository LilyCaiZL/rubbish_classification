...
add_arg('model_dir',        str,   'best_model',     "The model path.")
    ...
    if 'coco' in data_args.dataset:
        ...
        with codecs.open('label_list.txt') as f:
            for line in f:
                ...
                label_list[str(float(parts[0]))] = parts[1]
        print(label_list)
...
def draw_bounding_box_on_image(image_path, nms_out, confs_threshold,
    ...
    Font1 = ImageFont.truetype("Arial.ttf", 20, index = 0)
    for dt in nms_out:
        ...
        draw.line(
            ...
            fill=color)
        if image.mode == 'RGB':
            draw.text((left, top), label_list[int(category_id)], font = Font1,fill = color)
